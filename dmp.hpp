 ...
 * {Diff(DELETE, "Hello"), Diff(INSERT, "Goodbye"),
 *  Diff(EQUAL, " world.")}
 ...
 */
template <class String> class DMP {
...
public:
   ...
   */
  static constexpr float Diff_Timeout = 1.0f;
  /**
   * Cost of an empty edit operation in terms of edit characters.
   */
  static constexpr uint16_t Diff_EditCost = 4;
  /**
   * At what point is no match declared (0.0 = perfection, 1.0 = very loose).
   */
  static constexpr float Match_Threshold = 0.5f;
  /**
   * How far to search for a match (0 = exact location, 1000+ = broad match).
   * A match this many characters away from the expected location will add
   * 1.0 to the score (0.0 is a perfect match).
   */
  static constexpr int Match_Distance = 1000;
  /**
   ...
   */
  static constexpr float Patch_DeleteThreshold = 0.5f;
  /**
   * Chunk size for context length.
   */
  static constexpr uint16_t Patch_Margin = 4;
...
public:
  ...
  struct Diff {
     ...
     */
    Diff(Operation op, String txt) : operation{op}, text{txt} {
    }
    ...
    bool operator==(const Diff &b) const {
      return operation == b.operation && text == b.text;
    }

    string str() const {
      String p{text};
      //replace(p.begin(), p.end(), '\n', '\\');
      return op2str(operation) + "\t" + p;
    }
  };
...
public:
   ...
   * @return Linked std::vector of Diff objects.
   */
  std::vector<Diff> diff_main(String text1, String text2) {
   ...
   * @return Linked std::vector of Diff objects.
   */
  std::vector<Diff> diff_main(String text1, String text2, bool checklines) {
    // Set a deadline by which time the diff must be complete.
    long deadline;
    if (Diff_Timeout <= 0) {
      deadline = LONG_MAX;
    } else {
      // TODO deadline = System.currentTimeMillis() + (long) (Diff_Timeout *
      // 1000);
    }
  ...
  std::vector<Diff> diff_main(String text1, String text2, bool checklines,
    ...
    String commonprefix = text1.substr(0, commonlength);
    text1 = text1.substr(commonlength);
    text2 = text2.substr(commonlength);

    // Trim off common suffix (speedup).
    commonlength = diff_commonSuffix(text1, text2);
    String commonsuffix = text1.substr(text1.length() - commonlength);
   ...
   * @return Linked std::vector of Diff objects.
   */
  std::vector<Diff> diff_compute(String text1, String text2, bool checklines,
                                 long deadline) {
    std::vector<Diff> diffs{};
   ...
   */
  std::vector<Diff> diff_bisect(String text1, String text2, long deadline) {
   ...
   * @return std::vector of Diff objects.
   */
  std::vector<Diff> diff_bisectSplit(String text1, String text2, int x, int y,
                                     long deadline) {
    String text1a = text1.substr(0, x);
    String text2a = text2.substr(0, y);
    String text1b = text1.substr(x);
    String text2b = text2.substr(y);
 ...
 */
  ...
  */
  ...
  void diff_cleanupSemantic(std::vector<Diff> diffs) {
    ...
    bool changes = false;
    std::deque<Diff> equalities = new ArrayDeque<Diff>();  // Double-ended queue
  of qualities. String lastEquality = null; // Always equal to
  ...
  */
  ...
  void diff_cleanupEfficiency(std::vector<Diff> diffs) {
    ...
    bool changes = false;
    std::deque<Diff> equalities = new ArrayDeque<Diff>();  // Double-ended queue
  of equalities. String lastEquality = null; // Always equal to
  equalities.peek().text std::vectorIterator<Diff> pointer =
  diffs.listIterator();
  ...
  */
   ...
   * @return Source text.
   */
  String diff_text1(std::vector<Diff> diffs) {
    String text{};
   ...
   */
  String diff_text2(std::vector<Diff> diffs) {
    String text{};
    for (Diff aDiff : diffs) {
